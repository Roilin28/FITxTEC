...
export default function RoutineDetailsScreen() {
  ...
  const { user } = useAuth();
  const { routineId, isAiRoutine } = route.params as {
    routineId: string;
    isAiRoutine?: boolean;
  };
  ...
  useEffect(() => {
    const fetchRutina = async () => {
      setLoading(true);
      try {
        const data = isAiRoutine
          ? await getSavedRoutineDeep(routineId)
          : await getRutinaDeep(routineId);
        setRutina(data);
      } catch (e) {
    ...
    fetchRutina();
  }, [routineId, isAiRoutine]);

  const handleDeleteAiRoutine = async () => {
    if (!isAiRoutine) return;
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            setDeleting(true);
            try {
              await deleteSavedRoutine(routineId);
              Alert.alert(
                "Rutina Eliminada",
              ...
              console.error("Error al eliminar rutina AI:", e);
              Alert.alert(
                "Error",
                "No se pudo eliminar la rutina. Intenta de nuevo."
              );
            } finally {
              setDeleting(false);
            }
    ...
    <SafeAreaView style={styles.safe}>
        ...
        >
          ...
          <View style={styles.statsRow}>
            <View style={styles.statItem}>
              ...
              />
              <Text style={styles.statText}>{rutina.cantidadDias} días</Text>
            </View>
            <View style={styles.statItem}>
              <Ionicons
                name="time-outline"
                ...
                color={colors.textMuted}
              />
              <Text style={styles.statText}>{rutina.tiempoAproximado}</Text>
            </View>
            <View style={styles.statItem}>
              <Ionicons
                ...
                color={colors.textMuted}
              />
              <Text style={styles.statText}>{totalExercises} ejercicios</Text>
            </View>
          </View>
                ...
                styles.difficultyBadge,
                {
                  backgroundColor:
                    rutina.nivelDificultad === "Beginner"
                      ? "#22D3EE"
                      : rutina.nivelDificultad === "Intermediate"
                      ? "#F59E0B"
                      : "#EF4444",
                },
              ]}
      ...
      <SafeAreaView edges={["bottom"]} style={styles.actionButtonsContainer}>
        <View style={styles.actionButtonsRow}>
          {isAiRoutine ? (
            // Para rutinas AI: botón Eliminar + Iniciar Rutina
            <>
              <TouchableOpacity
                style={styles.deleteButton}
                onPress={handleDeleteAiRoutine}
                disabled={deleting}
              >
                {deleting ? (
            ...
            // Para rutinas normales: solo Iniciar Rutina (ocupa todo el ancho)
            <TouchableOpacity
